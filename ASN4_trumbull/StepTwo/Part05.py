...
def threshold_video(input_vid, mask, output, methods_num):
    cap_rgb = cv.VideoCapture(input_vid)

    template = cv.imread(mask, 0)
    ...
    while (cap_rgb.isOpened()):
        ...
        method = eval(methods[methods_num])
